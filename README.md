...
* [Round 28: actual prime matrices and their Mellin structure](research/reports/dyson_round28.md). Three predeclared genuine-prime matrices have selected Mellin transpose pairings at 35–55% of their finite operator norms. Full arrays and independent arithmetic/eigenpair checks are retained. An exact Mellin/zero test and a separate finite Gaussian theorem distinguish uniform norms from fixed-vector estimates; no actual asymptotic bound follows.
* [Round 27: remove a central divisor band and evaluate the pure divisor moments](research/reports/dyson_round27.md). Ordinary RH makes the actual central divisor band from X^(1/3) to X^.49 negligible. The complete pure approximant correlation has a finite arithmetic main with globally vanishing error. The genuine-prime mixed term, projection upper bound and joint matrix estimate remain open; full independent reviews and the precise 186 source transfer are preserved.
