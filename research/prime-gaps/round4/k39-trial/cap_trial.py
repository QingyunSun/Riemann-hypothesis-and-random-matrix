...
def geometry():
    ...
    rs = rho - gap
    S, T1 = Q(2742997, 10**7) / rs, Q(251, 1000) / rs
    T0 = Q(1997, 1000) - S
    epsilon = gap / rho
    ...
    shells = {
        "outer": [(new[0]["a"], zeta), (new[24]["a"], (S+epsilon)/2),
                  (S, S+epsilon/2-Q(23,40)*(T1+epsilon/2))],
        "base": [(old[12]["b"], zeta), (old[24]["b"], (T0+epsilon)/2),
                 (T0, Q(63,160)*(T0+epsilon/2))],
        "enlarged": [(new[12]["b"], zeta), (new[24]["b"], (T1+epsilon)/2),
                     (T1, Q(63,160)*(T1+epsilon/2))],
        "full": [(S, zeta)],
    }
    return {"rho": rho, "rho_star": rs, "S": S, "T0": T0, "T1": T1, "shells": shells}
...
class Trial:
    def __init__(self, k, intervals, tilt, dtype="longdouble"):
    ...
    def forms(self):
        ...
        for layer,cap in enumerate(self.caps):
            ...
            self.clear()
        mass,pair,lam=self.dtype(".99998"),self.dtype(".34"),self.dtype(".008")
