...
**最新检查点：2026-09-05，协调任务已完成第十八至二十一轮所提交证明的普通审查；这几轮不等同于外部审稿或 Lean 验证。** 当前已核实的最新公开版本为[第二十一轮提交 `6c49d118fb398a39b9e6e2a1d362a223150e80fd`](https://github.com/QingyunSun/Riemann-hypothesis-and-random-matrix/blob/6c49d118fb398a39b9e6e2a1d362a223150e80fd/research/reports/dyson_round21.md)。公开报告已全文核对；85 份公开原始文件已独立下载并核对清单哈希及字节数，四份最终作者稿件另与已审查的冻结副本逐字节比较。第二十轮的 64 份公开原始文件和全部 49,152 行计算分段也已完成此前核验。第二十一轮保留了逐位移假设的实质纠错；增长位移集合与新的中心化方案仍在研究，不计入本次审查结论。原始材料提取已经交付；后续实例来自本项目推导。当前最明确的目标是在 RH 下证明一个实际的正平均素数方差严格低于 AH 的饱和值；现在已有精确的带符号素数对余项和实际对数素数热能量两种表述。该严格不等式与原 Bragg 缺口有正上极限等价，但两者均尚未证明。
