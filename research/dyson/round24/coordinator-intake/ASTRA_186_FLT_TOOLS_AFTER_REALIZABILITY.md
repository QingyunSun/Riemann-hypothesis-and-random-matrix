...
2026-09-05，第二轮材料接入之后，Astra 提交了两份完整证明，协调者已独立审查通过：在全体奇数端点 m<n 上，单一或两个完整 Pareto 尾不能实现非零的精确加权零行列和；进一步，任意有限个不同指数、由紧支撑下端窗口生成的完整 Pareto 尾也不能实现这种方向。后一结论使用“下端系数最终为常数”的实际窗口结构，覆盖所有这类有限窗口选择，而非一次有限基底数值试验。

**第 2.1 节的有限棋盘格恒等式仍正确，但不可通过上述现有核族无代价实现。这一整类精确消去的窗口参数搜索应停止，不交给 Fable 扩大扫描。** 本文件下面保留原构造与推导，执行要求以本更新为准。

排除范围不包含独立的上端截断、具有可控误差的近似边缘消去，或三个以上指数的任意非最终常数行系数；这些可能性尚没有原目标比较或严格双素数估计，不能据此自动启动另一套搜索。当前优先继续真实的带符号 Möbius–素数相关和。

审查依据：一/二尾作者稿 SHA256 818f0ac7b2952fd05fde28762079aeb94d23c72fbe8171e3298cedeb935bb60c；有限实际窗口扩展稿 SHA256 c857d22a766099049bcdc95cdb12bf876d87deb6d83f9e9654c154d4dc2a6a7d；协调者完整普通审查 SHA256 4808d9b1a571e18c6852d534a10edf91d3314df3d8365cdad35109907950ea14。未运行新数值实验、外部模型或 Lean。没有证明严格方差缺口。

初始 17,120 字节任务书及其原 SHA256 331e923f508fe05f88491cb0b0992930323c2b095fe48a4083dd427c02ffa9da 已保留在冻结的 R23 输入中；本更新不改写该历史版本。
...
2. **停止已被排除的有限实际窗口精确零边缘搜索。** 第 2 节有限恒等式保留；若考虑排除范围以外的新核，先给出原目标比较与误差预算，再决定是否计算。当前没有这样的比较时，继续真实 Möbius–素数主线，不为绕过排除结果另开泛型工程。
