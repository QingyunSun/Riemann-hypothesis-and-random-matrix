...
**最新检查点：2026-09-05，协调任务已完成第十八至二十轮所提交证明的普通审查；这几轮不等同于外部审稿或 Lean 验证。** 当前已核实的最新公开版本为[第十九轮提交 `974a2f15fff1f2754e7f54739b6360ab4d8eb08a`](https://github.com/QingyunSun/Riemann-hypothesis-and-random-matrix/blob/974a2f15fff1f2754e7f54739b6360ab4d8eb08a/research/reports/dyson_round19.md)；其报告与四份最终证明已独立下载并与已审查稿件的 SHA-256 核对一致。第二十轮正在由主任务单独整合发布，下面按已审查的作者文件哈希记录。原始材料提取已经交付；后续实例来自本项目推导。当前最明确的目标是在 RH 下证明一个实际的正平均素数方差严格低于 AH 的饱和值。该严格不等式与原 Bragg 缺口有正上极限等价，但两者均尚未证明。
...
原始 FLT 与 186 材料提供了构造、分解和核验方法；上述新实例是本项目后续独立推导与普通证明审查的结果，不应倒称为原文已经证明的 RH 工具。协调任务已全文核对本文件所述第十六至二十轮的相应证明，核查关键原始定理范围，并对三点有限计算阅读代码、核对文件及 CSV 汇总。未重新运行全部计算，未完成 Lean 形式化、外部审稿或全局新颖性判定。

当前主任务应继续证明实际正平均素数方差的严格下降，或等价地证明原 Bragg 缺口具有正上极限。指数平均已经消除了这项新统计量的转换常数损失，高度连续性补上了两个严格目标之间的反向联系；下一步必须提供新的算术估计。暂缓更多形式推广、重复常数优化、扩大小规模扫描和大型 PDF 重建；保持一个既有 Fable 会话、一次一个明确任务，本文件不启动新的 Fable 工作。
