...
\tag{28}
\]
Then \(\alpha,\beta\) are genuine divisor-bounded coefficient sequences at scales \(D,K\). The **untwisted** \(\beta\) has the Siegel–Walfisz property required by Definition 2.9 of the 186 paper. For small \(q\), inclusion–exclusion for the auxiliary coprimality condition and the fixed odd restriction, followed by partial summation, leaves an endpoint error
