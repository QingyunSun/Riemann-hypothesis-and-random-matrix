...
The identity (5) is the sharp target for this particular statistic. A uniform absolute error estimate is a stronger sufficient hypothesis, useful for auditing proposed source applications.
...
\tag{32}
\]
Indeed \(\sum_{h\ge1}(1+h/(2X))^{-T}\le2X/(T-1)\). Summing \(O(\log T)\) blocks costs at most one additional logarithm. Thus a uniform \(\beta<4/9\), with fixed logarithmic loss, would imply \(\mathcal E_T=o(1)\) and the stronger conclusion \(\overline V_T\to M<1\). This hypothesis is not asserted.
...
The ordinary results here are (12), the exact centered kernel (14), the uniform singular-series identity (17), the reduction (5), and the Abel norm (29). The numerical-looking power thresholds in (33)–(36) are exact rational calculations. Nothing proves that \(\mathcal E_T\) has the sign or upper constant in (6).

The small checker verifies the kernel factorization on exact rational finite signed data, the floor-centering convention, the beta moments at several integer \(T\), and all displayed exponent thresholds. It reads the inherited bump diagnostic for (7); it does not calculate primes at new heights or replace the ordinary proof of (17). Source PDFs/text are retained locally with versioned URLs and hashes; only our report/checks/provenance should be published by the coordinator.
