...
\(O_f(\delta_*X+X(\log\log X)^2/\log X)\) for that summed maximal-residue norm. Pairing the \(H\)-packet by this norm gives \(O_{f,V}(\delta_*HX+\cdots)\), not the target scale. The source warns that its unspecified constant can make a particular positive \(\delta\) uninformative.
