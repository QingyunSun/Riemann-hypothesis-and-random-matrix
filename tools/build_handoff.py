...
REPORTS = [
    ...
    "transferable_tools_ritz_decision.md", "dynamic_generator.md",
    "dynamic_generator_independent_review.md", "force_energy.md",
    "force_energy_independent_review.md", "new_attachment_bridge_audit.md",
    "prime186_structural_frontier.md",
    "fable_task001_sync_review.md", "fable_heat_sync_review.md",
]
HISTORY = [
...
def cleaned(raw: str) -> str:
    # Reflow two wide historical proposal diagrams; source files remain verbatim.
    raw = raw.replace(
        r"""\text{bad adversarial state}
\rightarrow
...
def main() -> None:
    parts = [(ROOT / "docs/handoff/ASTRA_RESEARCH_HANDOFF_MAIN.md").read_text(),
             (ROOT / "docs/handoff/ASTRA_ROUND3_UPDATE.md").read_text(),
    ...
    entries += [(ROOT / "fable/overnight-2026-09-05" / name, "Fable source; scope set by current audit") for name in
                ("CLAIMS.md", "FABLE_COORDINATION.md", "r1_h2_interval_cert.md")]
    entries += [(ROOT / "research/incoming" / name, "Incoming proposal; claims require audit") for name in
                ("dynamic_generator_proposal.md", "heat_depth_classification_proposal.md", "hard_gap_depth_bridge_proposal.md")]
    ...
    print(json.dumps({"output": str(OUT), "included_documents": len(index) + 3, "bytes": OUT.stat().st_size}))
